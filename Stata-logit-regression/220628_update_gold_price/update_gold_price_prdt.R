...
Panel_T_MT <- subset(Panel_T_MT, select = -c(OBJECTID))


## get exchange rate and US CPI deflated price
gold_price_USCPI <- subset(gold_prod_prices_2, select = c(year, gold_price_real_USCPI))
price_ex_rate <- merge(gold_price_USCPI, exchange_rate, by = c("year"))
...
write_dta(Panel_C_M, "Panel_C_M.dta")
write_dta(Panel_C_T, "Panel_C_T.dta")
